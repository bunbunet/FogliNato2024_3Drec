...
MontDirJ=dirOut+File.separator+"MontageJPG"+File.separator;
MontDirT=dirOut+File.separator+"MontageTIF"+File.separator;
File.makeDirectory(MontDirJ);
File.makeDirectory(MontDirT);
...
for (i = 1; i < series+1; i++) {
	...
	saveAs("tiff", MontDirT+title+"series_"+i);
	saveAs("jpeg", MontDirJ+title+"series_"+i);
